...
function change_profile_image($user_id, $file_temp){
	$file_path = 'images/profile/' . $_FILES['profile']['name'];
...
function mail_users($subject, $body){
	$query = mysql_query("SELECT `email`, `first_name` FROM `users` WHERE `allow_mail` = 1");
	while(($row = mysql_fetch_assoc($query)) !== false){
	email($row['email'], $subject, "Hello " . $row['first_name'] . ",\n\n" . $body);
	}
}  

function has_access($user_id, $user_level){
  $user_id = (int)$user_id;
  $user_level = (int)$user_level;
  return (mysql_result(mysql_query("SELECT COUNT(`user_id`) FROM `users` WHERE `user_id` = $user_id AND `user_level` = $user_level"), 0) == 1) ? true : false;	
}	
...
function activate($email, $email_code){
	...
	if(mysql_result(mysql_query("SELECT COUNT(`users_id`) FROM `users` WHERE `email` = '$email' AND `email_code` = '$email_code' AND `active` = 0"), 0) == 1){
